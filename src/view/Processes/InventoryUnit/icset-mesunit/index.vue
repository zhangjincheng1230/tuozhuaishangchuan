<template>
  <div class="app-main">
    <div class="kj-container">
      ...
      <div class="kj-main">
        <div class="table-container">
          <el-table
            ...
            <el-table-column property="jldwbm" label="编码" width="180" header-align="center" align="left"></el-table-column>
            <el-table-column property="jldwmc" label="名称" width="150" header-align="center"  align="left"> </el-table-column>
            <el-table-column property="bz" label="备注" header-align="center" align="left" width="240"></el-table-column>
          </el-table>
        </div>
      ...
      <InventoryUnitDialog :visible.sync="dialogFormVisible" :title="dialogFormTitle" :data="formData" @save="onSaved"></InventoryUnitDialog>
...
import InventoryUnitDialog from './components/InventoryUnitDialog'
...
export default {
  ...
  computed: {
    sortedData () {
      return orderBy(this.tableData, v => v.jldwbm)
    },
    uploadParam () {
      return {
  ...
  methods: {
    ...
    fetchData (flag) {
      this.loading = true
      this.$api.InventoryManageApi.IcsetMesunit.queryInfo(this.getParam()).then(result => {
        this.loading = false
        if (result.success) {
          this.tableData = result.rows
          this.total = result.total
          this.initFilter(this.tableData, 'jldwmc')
    ...
    addInfo () {
      this.formData = Object.assign({}, { jldwbm: '001' })
    ...
    delRows (rows) {
      ...
      if (rows && rows.length > 0) {
        ids = rows.reduce((id, row) => id + ',' + row.id, '')
      } else {
        MsgUtil.warnMsg(this, '请选择要删除的数据！')
        return
      }
      ...
      this.$confirm('确定要删除选中的数据？', '提示', {
        ...
        this.$api.InventoryManageApi.IcsetMesunit.onDelete(param).then(result => {
          loading.close()
          if (result.success) {
            if (!result.msg) {
    ...
    onUpload (formData) {
      ...
      this.$api.InventoryManageApi.IcsetMesunit.impExcel(formData).then(result => {
        loading.close()
        if (result.success) {
          if (!result.msg) {
    ...
    expExcel () {
      this.$api.InventoryManageApi.IcsetMesunit.expExcel()
    },
