...
<!--        <el-button  v-if="disabled" @click="cancelEdit">下一页</el-button>-->
    ...
    <div class="kj-main">
      <el-form
        ...
        <el-row>
          <el-col :span="7">
            <el-form-item size="mini" prop="cbusitype"  label="入库类型">
          ...
          <el-col :span="10">
            ...
            <el-form-item size="mini" prop="iszg"  v-if="headData.cbusitype === '41' && !headData.isczg">
              <el-checkbox v-model="headData.iszg" @change="zgChecked" :disabled="disabled" style="margin-left: 50px">是否暂估</el-checkbox>
            </el-form-item>
            <el-form-item size="mini" prop="isczg"  v-if="headData.cbusitype === '41' && headData.isczg">
              <el-checkbox v-model="headData.isczg"  :disabled="true" style="margin-left: 50px">冲销暂估</el-checkbox>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row  v-if="headData.cbusitype === '41'">
          <el-col :span="7">
          ...
          <el-col :span="7">
            <el-form-item size="mini" prop="invid" label="发票号">
              <el-input v-model.trim="headData.invid" style="width: 185px;" :disabled="headData.iszg || disabled"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item size="mini" prop="invdate" label="发票日期">
              <el-date-picker type="date"  value-format="yyyy-MM-dd"  clearable :editable="false" v-model="headData.invdate"  style="width: 185px;" :disabled="headData.iszg || disabled"></el-date-picker>
            </el-form-item>
            <el-form-item size="mini" prop="isrz1">
              <el-checkbox v-model="headData.isrz1" :disabled="headData.iszg || disabled" style="margin-left: 50px">是否认证</el-checkbox>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row  v-if="headData.cbusitype === '41'">
          ...
          <el-col :span="7">
            <el-form-item size="mini" prop="pk_cust" label="供应商">
          ...
          <el-col :span="10">
            <el-form-item size="mini" prop="fp_style" label="发票类型">
              <el-select v-model="headData.fp_style"  @change="styleChange" :disabled="headData.iszg || disabled" style="width: 185px;">
          ...
          <el-table
            ...
            <el-table-column prop="num" label="数量" header-align ="center" align="right" width="120">
              <template #default="{ row, $index }">
                <TableFormItem :prop="'children.' + $index+'.num'">
                  <el-input-number v-model="row.num" :precision="numPre" :controls =false @change="numChange($index,row)"  :min="-99999999" :max="999999999" :disabled="disabled"></el-input-number>
                </TableFormItem>
              </template>
            </el-table-column>
            <el-table-column prop="price" label="单价" header-align ="center"  align="right" width="120">
              <template #default="{ row, $index }">
            ...
            <el-table-column prop="mny" label="金额" header-align ="center" align="right"  width="120">
              <template #default="{ row, $index }">
            ...
            <el-table-column  v-if="!headData.iszg" prop="ntax" header-align ="center" label="税率(%)" align="right" width="70">
              <template #default="{ row, $index }">
            ...
            <el-table-column v-if="!headData.iszg" prop="tmny" header-align ="center"   label="税额"  align="right" width="120">
              <template #default="{ row, $index }">
            ...
            <el-table-column v-if="!headData.iszg"  prop="ttmny"  label="税价合计" header-align ="center"   align="right"  width="120" ></el-table-column>
...
export default {
  ...
  computed: {
    ...
    cbusitypedata () {
      let cbusitypedata = IcbillManageConst.purchInType
      if (!this.disabled) {
        cbusitypedata = cbusitypedata.filter(item => (item.value !== '42' && item.value !== ''))
      }
      return cbusitypedata
    },
    disabled () {
  ...
  methods: {
    ...
    addRow: function () {
      this.headData.children.splice(this.headData.children.length + 1, 0, { ntax: 13 })
    },
    removeRow: function () {
    ...
    saveForm (auto) { // 保存
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          ...
          this.$api.InventoryManageApi.IcbillPurchin.saveData(temp).then(result => {
            this.closeLoading()
            if (result.success) {
    ...
    loadBillById (id) {
      ...
      this.$api.InventoryManageApi.IcbillPurchin.editData({'id_ictrade_h': id, 'ignoreCheck': 'Y'}).then(res => {
    ...
    typechange (type) {
      if (IcbillManageConst.purchInType[1].value !== type) {
        ...
        this.headData.isrz1 = false
        this.headData.iszg = false
      }
    ...
    zgChecked (check) {
      ...
      this.styleChange(this.headData.fp_style)
      this.$nextTick(() => {
        this.$refs.addTableRef.doLayout()
      })
    },
    ...
    styleChange (style) {
      if (this.$parent.chargedeptname() && style === '2') {
        this.headData.isrz1 = true
      } else {
        this.headData.isrz1 = false
      }
    },
    ...
    getSummaries (param) {
      ...
      const sums = []
      let excludeInx = [0, 1, 3, 4, 6, 8, 11, 12, 13]
      if (this.headData.iszg) { excludeInx = [0, 1, 3, 4, 6, 8, 9, 10] }
