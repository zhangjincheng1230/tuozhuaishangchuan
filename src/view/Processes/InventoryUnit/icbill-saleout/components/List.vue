  ...
  <div class="kj-container" >
    <div class="kj-head">
      <div class="query-info">期间
        ...
        <span class="query-corp" style="min-width:60px"></span>
        <el-switch  style="margin-left: 10px;margin-top:4px" v-model="ishowcust"
                    active-value= true inactive-value= false active-text="显示客户" @change="saveSetting"></el-switch>
      </div>
      <div class="kj-button-group">
        ...
        <el-button  style="margin-left:10px" @click="onRedBack">红字冲回</el-button>
        <el-dropdown style="margin-left:10px">
    ...
    <div class="kj-main">
      <div class="table-container">
        <el-table
          ...
          <el-table-column prop="cbusitype" align="center"  label="出库类型"  width="80" :formatter="typeformatter"></el-table-column>
          ...
          <TableColumnPre prop="vdef2" label="成本单价" :pretype = 2></TableColumnPre>
          <TableColumnPre prop="vdef3" label="成本金额"></TableColumnPre>
          <TableColumnPre prop="vdef4" label="销售单价" :pretype = 2></TableColumnPre>
          <TableColumnPre prop="mny" label="销售金额"></TableColumnPre>
          <TableColumnPre prop="tmny" label="税额" ></TableColumnPre>
          <TableColumnPre prop="ttmny" label="价税合计" ></TableColumnPre>
          <el-table-column prop="isbackname" header-align ="center" align="center" label="红字冲回" width="100">
            <template slot-scope="scope">
              <span   class="spanlink" >{{scope.row.isbackname}}</span>
            </template>
          </el-table-column >
          <el-table-column v-if="this.ishowcust==='true'"  prop="custname" align="center" label="客户" show-overflow-tooltip width="150" > </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    fetchData (isQuery) {
      ...
      this.$api.InventoryManageApi.IcbillSaleout.queryData(param).then(result => {
        this.loading = false
        if (result.success) {
          ...
          if (isQuery) { MsgUtil.successMsg(this, '查询成功') }
        } else {
    ...
    onAdd () {
      ...
      const busitype = IcbillManageConst.saleOutType[1].value
      let param = {
        ddate: date,
        cbusitype: busitype
      }
      this.$api.InventoryManageApi.IcbillSaleout.getBillNo(param).then(result => {
        if (!result.success) {
          ...
          this.$api.InventoryManageApi.IcsetSetting.query().then(res => {
            const formData = {curOptType: 'new', 'id_corp': this.$parent.pk_corp, did: result.data, cbusitype: busitype, ddate: date, iway: res.rows.xsfkfs + '', iszg: false, isczg: false, isback: false, isrz1: false, isjc: false}
            if (this.$parent.chargedeptname()) {
              formData.fp_style = '2'
            } else {
              formData.fp_style = '1'
            }
            formData.children = [{ ntax: this.$parent.getNtax() }]
    ...
    editRow (rowData, isEdit) {
      if (rowData) {
        // 调用后台的业务逻辑
        this.$api.InventoryManageApi.IcbillSaleout.editData({'id_ictrade_h': rowData.id_ictrade_h}).then(result => {
          if (!result.success) {
            if (isEdit) {
              MsgUtil.errorMsg(this, '已经转总账数据不能修改！')
            } else {
              this.$api.InventoryManageApi.IcbillSaleout.editData({'id_ictrade_h': rowData.id_ictrade_h, 'ignoreCheck': 'Y'}).then(result => {
                if (result.success) {
    ...
    onCopy () {
      ...
      // 调用后台的业务逻辑
      this.$api.InventoryManageApi.IcbillSaleout.copeData({'id_ictrade_h': id}).then(result => {
        if (!result.success) {
          ...
          this.$api.InventoryManageApi.IcbillSaleout.getBillNo(param).then(res => {
            if (!res.success) {
    ...
    onDelete () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$confirm('您确定删除该条数据?', '提示', {
          ...
          this.$api.InventoryManageApi.IcbillSaleout.delSale({head: JSON.stringify(this.listSelectedRows)}).then(result => {
            loading.close()
            if (result.success) {
    ...
    onRedBack () {
      ...
      const loading = MsgUtil.loading(this, '正在红字冲回...')
      this.$confirm('您确定红字冲回该条数据?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        iconClass: 'el-icon-warning-outline',
        ...
        this.$api.InventoryManageApi.IcbillSaleout.dashBack(rowData).then(result => {
          loading.close()
          if (result.success) {
            MsgUtil.successMsg(this, result.msg)
          } else {
    ...
    toVou () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$confirm(' 你确定要进行转总账吗?', '提示', {
          ...
          this.$api.InventoryManageApi.IcbillSaleout.togl({head: JSON.stringify(this.listSelectedRows)}).then(result => {
            loading.close()
            if (result.success) {
              MsgUtil.successMsg(this, result.msg)
            } else {
    ...
    toVous () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$confirm('你确定要进行转总账吗?', '提示', {
          ...
          this.$api.InventoryManageApi.IcbillSaleout.toTotalGL({head: JSON.stringify(this.listSelectedRows)}).then(result => {
            loading.close()
            if (result.success) {
              MsgUtil.successMsg(this, result.msg)
            } else {
    ...
    cancelToVoU () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$confirm('你确定要进行取消转总账吗?', '提示', {
          ...
          this.$api.InventoryManageApi.IcbillSaleout.rollback({head: JSON.stringify(this.listSelectedRows)}).then(result => {
            loading.close()
            if (result.success) {
              MsgUtil.successMsg(this, result.msg)
            } else {
    ...
    onUpload (formData) {
      ...
      this.$api.InventoryManageApi.IcbillSaleout.impExcel(formData).then(result => {
        loading.close()
        if (result.success) {
          if (result.msg) {
            MsgUtil.errorMsg(this, result.msg)
    ...
    expExcelemplate () {
      let list = 'download'
      this.$api.InventoryManageApi.IcbillSaleout.expExcel({'list': JSON.stringify(list)})
    },
    expExcel () {
      if (this.IsSelectCurrentRow()) {
        let ids = ''
        ...
        this.$api.InventoryManageApi.IcbillSaleout.expExcel({'list': JSON.stringify(ids)})
      }
    },
    setPrint (param) {
      this.printParam = param
      this.ishowcust = (this.printParam.isshow === 'Y' || this.printParam.isshow === 'true') ? 'true' : 'false'
      this.$parent.printForm = param
    },
    // 保存设置
    saveSetting () {
    ...
    confirmPrint (printData) {
      ...
      this.$api.InventoryManageApi.IcbillSaleout.printPdf(printData)
    ...
    typeformatter (row, column, cellValue, index) {
      if (cellValue) {
        const item = IcbillManageConst.saleOutType.find(item => item.value === cellValue)
        return item && item.label ? item.label : IcbillManageConst.saleOutType[1].label
      }
      return IcbillManageConst.saleOutType[1].label
    },
    getSummaries (param) {
      const { columns, data } = param
      const sums = []
      const excludeInx = [0, 1, 3, 4, 5, 7, 9, 13]
