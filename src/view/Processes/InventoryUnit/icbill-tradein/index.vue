...
export default {
  ...
  methods: {
    ...
    fetchData () {
      ...
      this.$api.InventoryManageApi.IcbillTradein.query(param).then(result => {
        this.loading = false
        if (result.success) {
    ...
    confirmPrint (printData) {
      if (this.listSelectedRows && this.listSelectedRows.length > 0) {
        ...
        this.$api.InventoryManageApi.IcbillTradein.print(param)
      } else {
        this.$message({
          showClose: true,
          message: '未选中任何数据，请至少选择一行数据！',
    ...
    exportReport () {
      if (this.listSelectedRows && this.listSelectedRows.length > 0) {
        ...
        this.$api.InventoryManageApi.IcbillTradein.expExcel(param)
      } else {
        this.$message({
          showClose: true,
          message: '未选中任何数据，请至少选择一行数据！',
