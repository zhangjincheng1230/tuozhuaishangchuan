<template>
  <div style="position: relative;border: 1px solid #ddd;background:rgba(255,255,255,1)" class="output">
    <el-container>
      <el-form
        :inline="true"
          ...
          <el-row class="form-row">
            ...
            <el-col :span="10" class="item">
              <el-form-item label="名    称" size="mini" prop="id">
                <el-select v-model="adddoc.header.skhmc" filterable allow-create
                           default-first-option :disabled="isEidt" @change="ghfmcChange" style="width: 295px">
                  <el-option
                    v-for="item in ghfmcOptions"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
          ...
          <el-row class="form-row">
            ...
            <el-col :span="10" class="item">
              <el-form-item label="名  称" prop="xhfmc">
                <el-input v-model="adddoc.header.xhfmc" :disabled="isEidt" style="width: 295px" size="mini" placeholder="个人企业名称" />
              </el-form-item>
      ...
      <el-main style="padding: 0">
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="本期销售数量">
            <template slot-scope="scope">
              <span v-if="scope.row.isAdd === '是'" style="color: red">{{getThousands(scope.row.saleNumber, 0)}}</span>
              <span v-else-if="isKc && !scope.row.chid" style="color: red">{{getThousands(scope.row.saleNumber, 0)}}</span>
              <span v-else>{{getThousands(scope.row.saleNumber, 0)}}</span>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
            ...
            label="本期销售单价">
            <template slot-scope="scope">
              <span v-if="scope.row.isAdd === '是'" style="color: red">{{getThousands(scope.row.salePrice, 1)}}</span>
              <span v-else-if="isKc && !scope.row.chid" style="color: red">{{getThousands(scope.row.salePrice, 1)}}</span>
              <span v-else>{{getThousands(scope.row.salePrice, 1)}}</span>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
  ...
  methods: {
    ...
    InventoryRow: function (param) {
      if (param.dataRow !== undefined) {
        this.stockCurrentRow.name = param.dataRow.fullName !== undefined ? param.dataRow.fullName : param.dataRow.spmc
    ...
    onUpload (formData) {
      ...
      this.$api.outPutBill.impExcel(formData).then(result => {
        loading.close()
        if (result.success) {
    ...
    expExcel () {
      this.$api.outPutBill.exportExcelData('销项发票', {})
    },
    confirm: function () {
      this.loadTableList()
      this.getDateToReturn()
    ...
    addData: function () {
      this.$nextTick(() => {
        ...
        this.getGoodsInfo()
        this.getSellerInfo()
        this.$api.outPutBill.queryCorpReference({}).then(result => {
          if (result.success) {
            this.adddoc.header.xhfmc = result.data.corpname !== undefined ? result.data.corpname : ''
            this.adddoc.header.xhfsbh = result.data.taxnum !== undefined ? result.data.taxnum : ''
            this.adddoc.header.xhfdzdh = result.data.addressphone !== undefined ? result.data.addressphone : ''
            this.adddoc.header.xhfyhzh = result.data.banknum !== undefined ? result.data.banknum : ''
          }
        })
      })
    },
    ...
    querySaveBillForm: function (formName, val) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$api.outPutBill.saveOrUpdate(param).then(result => {
            loading.close()
    ...
    // 购买方名称
    ghfmcChange: function (val) {
      this.ghfmcOptions.forEach(item => {
        if (item.id === val) {
          this.adddoc.header.skhmc = item.name !== undefined ? item.name : ''
          this.adddoc.header.khsbh = item.credit_code !== undefined ? item.credit_code : ''
          this.adddoc.header.ghfdzdh = item.address !== undefined ? item.address : ''
          this.adddoc.header.ghfdzdh += this.adddoc.header.ghfdzdh !== '' ? item.phone_num !== undefined ? '  ' + item.phone_num : '' : ''
          this.adddoc.header.ghfyhzh = item.account_num !== undefined ? item.account_num : ''
        }
      })
    },
    // 业务类型弹出框
    openBusinessType: function () {
      if (this.adddoc.header.skprj !== '') {
        let period = this.isAdd ? this.tableSelection[0].inqj : this.adddoc.header.skprj.substring(0, 7)
        this.$api.outPutBill.queryCategory({period: period}).then(result => {
          if (result.success) {
    ...
    getGoodsInfo: function () {
      ...
      this.$api.outPutBill.queryB(param).then(result => {
        if (result.success) {
    ...
    getSellerInfo: function () {
      let param = {
        id: '000001000000000000000001'
        // pk_corp: this.$store.getters.getLoginCorpId,
      }
      this.$api.outPutBill.customerInfoQueryB(param).then(result => {
        if (result.success) {
          this.ghfmcOptions = result.rows
        }
      })
    },
    // 修改
    editRow: function () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.getGoodsInfo()
        this.getSellerInfo()
        this.$api.outPutBill.queryInfoByID({id: this.tableSelection[0].id}).then(result => {
          loading.close()
          if (result.success) {
            this.dialogBill = true
    ...
    preview: function (row) {
      ...
      this.getGoodsInfo()
      this.getSellerInfo()
      this.$api.outPutBill.queryInfoByID({id: row.id}).then(result => {
        loading.close()
        if (result.success) {
    ...
    deleteRows: function () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$confirm('你确定要删除吗?', '提示', {
          ...
          this.$api.outPutBill.onDelte({head: JSON.stringify(this.tableSelection)}).then(result => {
            this.successMsg(result)
            if (result.success) {
              this.loadTableList()
            }
    ...
    openToPick: function () {
      ...
      this.$api.outPutBill.ticketWay({}).then(result => {
        if (result.success) {
    ...
    saveForm: function () {
      const loading = MsgUtil.loading(this)
      this.$api.outPutBill.onTicket(this.toPickForm).then(result => {
        loading.close()
        this.successMsg(result)
        if (result.success) {
    ...
    queryCreate: function () {
      ...
      const loading = MsgUtil.loading(this)
      this.$api.outPutBill.combinePZ(param).then(result => {
        loading.close()
        this.successMsg(result)
        if (result.success) {
          this.loadTableList()
    ...
    createProof: function () {
      if (this.IsSelectCurrentRow()) {
        ...
        if (loginCorp.buildic === '0') {
          ...
          const loading = MsgUtil.loading(this)
          this.$api.outPutBill.getGoods({head: JSON.stringify(this.tableSelection)}).then(result => {
            loading.close()
          ...
          this.isKc = false
          this.$api.outPutBill.cbhsTypeQuery({}).then(result => {
            if (result.success) {
              if (result.rows === undefined || result.rows.chcbjzfs === '-1') {
                ...
                const loading = MsgUtil.loading(this)
                this.$api.outPutBill.createPZData(param).then(result1 => {
                  loading.close()
    ...
    matchData: function (val, chcbjzfs) {
      const loading = MsgUtil.loading(this)
      this.$api.outPutBill.matchInventoryData({head: JSON.stringify(this.tableSelection), ishow: this.isShowyppch ? 'Y' : 'N'}).then(result => {
        loading.close()
        if (result.success) {
    ...
    savaInventory: function () {
      ...
      const loading = MsgUtil.loading(this)
      this.$api.outPutBill.saveInventoryData({goods: JSON.stringify(this.stockSelectedRow)}).then(result => {
        loading.close()
        if (result.success) {
          this.matchData()
        } else {
    ...
    queryMatStock: function (val) {
      ...
      if (val === 1) {
        ...
        this.$nextTick(() => {
          this.$api.outPutBill.createPzData(param).then(result => {
            loading.close()
            this.successMsg(result)
            if (result.success) {
    ...
    getSubAndStockInfo: function () {
      this.$api.outPutBill.queryBySpercialKM({}).then(result => {
        if (result.success) {
          this.subData = result.rows
        }
      })

      this.$api.outPutBill.queryB({id: '000001000000000000000006'}).then(result => {
        if (!result.success) {
          this.successMsg(result)
        }
        result.rows.forEach(item => {
    ...
    enterSetting: function () {
      if (this.IsSelectCurrentRow()) {
        ...
        this.$api.outPutBill.queryCategorySet(param).then(result => {
          loading.close()
          if (result.success && result.rows.length > 0) {
    ...
    saveSettingForm: function (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const loading = MsgUtil.loading(this)
          this.$api.outPutBill.updateCategory(this.SettingForm).then(result => {
            loading.close()
            this.successMsg(result)
            if (result.success) {
    ...
    getBusinessType: function (param) {
      this.$api.outPutBill.queryCategory({period: param}).then(result => {
        if (result.success) {
    ...
    handleCommand: function (command) {
      if (command === 'rule') {
        ...
        this.$api.outPutBill.queryRule({}).then(result => {
          if (result.success) {
        ...
        if (this.IsSelectCurrentRow()) {
          this.$api.outPutBill.exportExcelData('销项发票', {daterows: JSON.stringify(this.tableSelection)}).then(result => {
            if (result.success) {
              this.successMsg(result)
            }
          })
    ...
    saveMerge: function () {
      ...
      const loading = MsgUtil.loading(this)
      this.$api.outPutBill.combineRule(this.mergeForm).then(result => {
        loading.close()
        this.successMsg(result)
        if (result.success) {
    ...
    loadTableList: function () {
      ...
      this.loading = true
      this.$api.outPutBill.queryData(param).then(result => {
        this.loading = false
        if (result.success) {
