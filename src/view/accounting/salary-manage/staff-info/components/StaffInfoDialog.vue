    ...
    <div class="form-container" :style="'max-height: ' + maxHeight +'px'">
      <el-form
        ...
        <el-row >
          ...
          <div v-if="formData.zjlx === '身份证' || formData.zjlx === '护照' || formData.zjlx === '其他'">
            <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="部门" prop="cdeptid">
              <el-select  v-model="formData.cdeptid" filterable clearable
                ...
                <el-option v-for="item in deptData" :key="item.id" :label="item.code ? item.code + ' / ' + item.name : item.name" :value="item.id">
                </el-option>
              </el-select>
