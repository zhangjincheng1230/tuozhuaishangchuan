...
export default {
  ...
  methods: {
    ...
    calLjData (newValue, oldValue, column) {
      const temp = Number(newValue) - Number(oldValue) + Number(this.formData[column])
    ...
    calGz () {
      ...
      let ynssde = this.$parent.subByDouble(dous)
      ...
      let ynse = ynssde * shuilv / 100 - quickdeduction
      if (ynse < 0) { ynse = 0 }
      ...
      let sfgz = this.$parent.subByDouble(dous)
      if (sfgz < 0) { sfgz = 0 }
