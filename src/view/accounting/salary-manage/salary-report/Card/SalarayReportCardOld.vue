...
export default {
  ...
  methods: {
    ...
    calAdd (columns) {
      ...
      const zxdc = this.$parent.addByDouble(array)
      return zxdc
    },
    calGzOld () {
      const qj = this.parent.period
      const yfgz = this.formData.yfgz
      const yanglaobx = this.formData.yanglaobx
      const yiliaobx = this.formData.yiliaobx
      const shiyebx = this.formData.shiyebx
      const zfgjj = this.formData.zfgjj

      const total = Number(yanglaobx) + Number(yiliaobx) + Number(shiyebx) + Number(zfgjj)
      let ynssde = Number(yfgz) - total
      if (ynssde <= 0) { ynssde = 0 }

      const kcnum = this.$parent.getkcnum('01', qj)
      const taxs = this.$parent.getTax(ynssde, Number(kcnum))
      const slv = taxs[0]
      const quickdeduction = taxs[1]
      let grsds = ((ynssde - Number(kcnum)) * slv / 100 - quickdeduction)
      if (grsds <= 0) { grsds = 0 }
