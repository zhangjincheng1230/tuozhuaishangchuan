      ...
      <el-popover
        ...
        <el-form ref="form" :model="formParam" label-width="auto">
          ...
          <el-collapse-transition>
            <div v-if="showFullParam" style="max-height: 210px">
...
export default {
  ...
  methods: {
    init () {
      ...
      if (!this.$_.isEmpty(this.routerParam)) {
        if (this.routerParam.hasOwnProperty('id') &&
          (this.routerParam.id === 'fsyeb' || this.routerParam.id === 'kmzz')) {
          // this.formParam.kms_first = this.routerParam.kmbm
          // this.formParam.kms_last = this.routerParam.kmbm
          this.formParam.periodRange = this.routerParam.queryParam.periodRange
          this.formParam.corpobj = this.routerParam.queryParam.corpobj
        } else {
          Object.entries(this.routerParam).forEach(([key, value]) => {
    ...
    querySubjectRef () {
      this.subjectRef = []
      this.$api.kmmxReport.getSubjectRef(this.formParam.corpIds).then(
        result => {
          if (result.success) {
