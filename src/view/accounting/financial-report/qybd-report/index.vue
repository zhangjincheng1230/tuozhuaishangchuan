...
export default {
  ...
  methods: {
    ...
    query () {
      this.loading = true
      this.$api.qybdReport.query(this.getQueryParam()).then(result => {
        this.loading = false
    ...
    excelexport () {
      ...
      this.$api.qybdReport.exportExcel('权益变动表_' + this.queryParam.begindate, this.getExportParam(), function () {
        exportloading.close()
      })
    },
    getExportParam () {
    ...
    confirmPrint (val) {
      this.$api.qybdReport.printPdf('权益变动表', this.getPrintParam(val))
    },
    getPrintParam (val) {
      let printparam = {
        corpName: this.currentCorp.uname,
